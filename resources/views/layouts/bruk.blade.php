...
<head>
    ...
    <title>@yield('title', config('app.name', 'Current Financial Bank') . ' – Digital Banking Made Simple')</title>
    <meta name="description" content="@yield('meta_description', config('app.name', 'Current Financial Bank') . ' is a digital bank built for everyday people — open an account online, move money instantly, and grow your savings from one secure dashboard.')">
    <link rel="icon" type="image/png" href="{{ asset('bruk/img/favicon.webp') }}">

    <style>
        /* Preloader: larger, bolder brand mark */
        .preloader-area .waviy {
            font-size: 72px;
            font-weight: 800;
        }

        /* Keep nav legible when the navbar sits transparent over a dark hero (e.g. About Us breadcrumb) */
        .navbar-area.on-dark:not(.sticky) .navbar-nav > .nav-item > .nav-link,
        .navbar-area.on-dark:not(.sticky) .navbar-nav > .nav-item > .nav-link.dropdown-toggle:before,
        .navbar-area.on-dark:not(.sticky) .logo-light,
        .navbar-area.on-dark:not(.sticky) .link.style-four {
            color: #fff !important;
        }
        .navbar-area.on-dark:not(.sticky) .burger-menu span {
...
<body>
    ...
    <div class="preloader-area" id="preloader">
        <div class="loader">
            <div class="waviy">
                @foreach (str_split('CFB') as $letter)
                    <span>{{ $letter }}</span>
                @endforeach
            </div>
        </div>
    ...
    <!-- Navbar Area Start -->
    <div class="navbar-area style-three position-absolute top-0 start-0 w-100 @yield('navbar_style')" id="navbar">
        <div class="container pe-xxl-0">
            <nav class="navbar navbar-expand-lg d-flex justify-content-between align-items-center">
                <a href="{{ route('home') }}" class="logo d-lg-none">
                    <span class="logo-light fs-24 fw-bold text-title">{{ config('app.name', 'Current Financial Bank') }}</span>
                    <span class="logo-dark fs-24 fw-bold text-white">{{ config('app.name', 'Current Financial Bank') }}</span>
                </a>
    ...
    <footer class="footer-wrap position-relative bg-optional pt-130">
        ...
        <div class="container">
            <div class="footer-widget-wrap d-flex flex-wrap pb-100">
                <div class="footer-widget mb-25">
                    ...
                    <p class="comp-desc">{{ config('app.name', 'Current Financial Bank') }} is a digital bank built for everyday people — simple accounts, fast transfers, and real support, wherever you are.</p>
                </div>
                <div class="footer-widget mb-25">
        ...
        <p class="copyright-text text-center text-offwhite mb-0"><i class="ri-copyright-line"></i> {{ date('Y') }} <span class="text-white fw-semibold">{{ config('app.name', 'Current Financial Bank') }}</span>. All rights reserved.</p>
    </footer>
    <!-- Footer End -->
